...
using pair_allegro.cpp as a template
*/
...
PairMace::PairMace(LAMMPS *lmp) : Pair(lmp) {
  ...
  if(const char* env_p = std::getenv("MACE_DEBUG")){
    std::cout << "PairMace is in DEBUG mode, since MACE_DEBUG is in env\n";
    debug_mode = 1;
  }
    ...
    if(deviceidx >= 0) {
      ...
      if(deviceidx >= devicecount) {
        if(debug_mode) {
          ...
          std::cerr << "WARNING (Mace): my rank (" << deviceidx << ") is bigger than the number of visible devices (" << devicecount << "), wrapping around to use device " << deviceidx % devicecount << " again!!!";
          deviceidx = deviceidx % devicecount;
        }
        else {
          // Otherwise, more ranks than GPUs is an error
          std::cerr << "ERROR (Mace): my rank (" << deviceidx << ") is bigger than the number of visible devices (" << devicecount << ")!!!";
          error->all(FLERR,"pair_mace: mismatch between number of ranks and number of available GPUs");
        }
      }
    }
  ...
  std::cout << "Mace is using device " << device << "\n";
}
...
void PairMace::init_style(){
  if (atom->tag_enable == 0)
    error->all(FLERR,"Pair style Mace requires atom IDs");
  ...
  if (force->newton_pair == 0)
    error->all(FLERR,"Pair style Mace requires newton pair on");
}
...
void PairMace::settings(int narg, char ** /*arg*/) {
  // "mace" should be the only word after "pair_style" in the input file.
  if (narg > 0)
...
void PairMace::coeff(int narg, char **arg) {
  ...
  std::cout << "Mace: Loading model from " << arg[2] << "\n";
  ...
  if (model.hasattr("training")) {
    std::cout << "Mace: Freezing TorchScript model...\n";
  ...
  // std::cout << "Mace: Information from model: " << metadata.size() << " key-value pairs\n";
  ...
  std::cout << "Mace type | Mace name | LAMMPS type | LAMMPS name" << "\n";
...
#pragma omp parallel for reduction(+:nedges)
  ...
  if (debug_mode) printf("Mace edges: i j rij\n");
#pragma omp parallel for
  ...
  if (debug_mode) printf("end Mace edges\n");
